...
public class DongACrawling {
	...
	public static void main(String[] args) {
		...
		try {
			// 주소 만들기
			String addr = "http://www.donga.com/news/search?query="
					+ URLEncoder.encode("박문석", "utf8") + "&x=0&y=0";
			URL url = new URL(addr);
			// 연결하기
			HttpURLConnection con = (HttpURLConnection) url.openConnection();
			// 연결된 곳의 ContentType을 가져와서 EUC-KR이 포함되어 있다면
		...
		try {
			for(int i=0; i<pagesu; i=i+1) {
				// 주소 만들기
				String addr = "http://www.donga.com/news/search?p="
						+ (i*15 + 1) + "&query="
						+ URLEncoder.encode("박문석", "utf8")
				...
				Elements element = document.select("div > p.tit > a");
		...
		// 리스트에 저장된 링크를 가지고 기사를 찾아서 파일에 기사 내용을 저장하기
